...
const nameFileConfig = "nv-deploy.json"
...
export const factory = () => {
    ...
    if (!existsSync(join(nameFileConfig))){
        writeFileSync(nameFileConfig, JSON.stringify({ apps: [] }, undefined, 4));
    }

    let smConfig = JSON.parse(readFileSync(nameFileConfig).toString()) as IConfigWorkspace;
...
export const save = (values: { server?: IServer, cache?: { login?: { server: string, username: string } }, application?: IApplication, deleteServer?: string; }) => {
    ...
    if (values.application){
        ...
        writeFileSync(nameFileConfig, JSON.stringify(config, undefined, 4));
    }
